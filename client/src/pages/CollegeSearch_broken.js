...
const CollegeSearch = () => {
  ...
  const colleges = searchTerm.length > 2 ? searchData?.data : collegesData?.data || [];
  ...
  const handleAddToFavorites = async (collegeId) => {
    try {
      await collegeService.addToFavorites(collegeId);
  ...
  const getActiveFiltersCount = () => {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (key !== 'sortBy' && key !== 'sortOrder' && value && value !== '') {
        count++;
      }
    });
    return count;
      ...
      <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-4 mb-6">
        <div className="flex justify-between items-center">
          <p className="text-gray-300 font-medium">
            {isLoading || searchLoading ? 'Loading...' : `${colleges.length} colleges found`}
          </p>
          {getActiveFiltersCount() > 0 && (
            ...
            <div key={college.id} className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-yellow-400/10 hover:border-yellow-400/30 transition-all duration-300 transform hover:scale-[1.02] group">
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-700/50">
                  <button
                    onClick={() => handleAddToFavorites(college.id)}
